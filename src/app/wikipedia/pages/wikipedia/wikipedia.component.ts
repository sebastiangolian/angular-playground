...
export class WikipediaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  results: WikipediaResultOpensearch[]
  parseItem: WikipediaResultParse
  ...
  search(): Observable<WikipediaResultOpensearch[]> {
  ...
  parse(page: string): Observable<WikipediaResultParse> {
    return this.wikipediaService.parse(page)
  }

  onItemSelected(result: WikipediaResultOpensearch) {
    this.results = []
